              ...
              {loading ? (
                ...
                contentsByDay(d).map(c => (
                  <button
                    ...
                    className={`w-full text-left p-1.5 rounded-lg border text-xs transition-all hover:opacity-80 overflow-hidden ${
                      c.status === 'published'
                        ? 'bg-gray-50 border-gray-200 text-gray-500'
                        : STATUS_STYLE[c.status] ?? 'bg-gray-100 text-gray-600 border-gray-200'
                    }`}
                  >
                    <div className="flex items-center gap-1 mb-0.5 w-full min-w-0">
                      <Share2 className="w-3 h-3 flex-shrink-0" />
                      <span className="font-medium truncate flex-1 min-w-0">
                        {c.caption?.slice(0, 12) ?? ''}
                      </span>
                    </div>
                    {c.status === 'published' && (
                      <span className="inline-flex items-center gap-0.5 px-1 py-0.5 bg-green-100 text-green-600 rounded-full text-[9px] font-medium">
