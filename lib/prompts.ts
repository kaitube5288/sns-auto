...
export function buildThreadsPrompt(
  ...
  const trendStr = trendSummary
    ? `\n[오늘의 자영업 트렌드 — 글마다 다른 이슈 하나씩 배분 적용]\n- 아래 이슈들을 생성할 글 개수에 맞게 나눠서, 글 하나에 이슈 하나씩만 소재로 활용\n- 같은 이슈를 두 글에 중복 사용 금지\n${trendSummary}`
    : ''

  const hasThreadsStyle = tones.includes('스레드감성형')

  const threadsExamples = `
[한국 Threads 바이럴 글 실제 예시 - 구조가 모두 다름. 공식 없이 이 다양성 그대로]
...
예시6 (나열형): "오늘 한 일\n주문 밀려서 정신없이 뛰어다님\n진상 손님 처리\n마감 후 청소\n근데 계산해보니 역대급 매출\n인생 참"
예시7 (단상형): "카페 창업 3년차인데\n아직도 오픈 30분 전에 가슴이 뛰어\n설레서가 아니라 오늘은 손님이 와줄까 싶어서"
예시8 (팩트 시작형): "임대료 250만원\n인건비 320만원\n재료비 180만원\n\n이거 다 내고 나서 월급 받는 사람은\n저 혼자임"
예시9 (질문 시작형): "자영업 하면서 제일 힘든 게 뭔지 알아?\n\n돈 못 버는 것도 아니고\n몸이 힘든 것도 아니고\n\n아무도 알아주지 않는다는 거"`
...
- [상황]\n\n[반전 한마디] 공식 구조 반복 — 매 글에 같은 패턴 쓰지 말 것
- 매번 비슷한 글자 수 / 비슷한 이모지 위치 / 비슷한 줄바꿈 패턴
...
[글 구조 다양화 — 공식 반복 금지]
- 첫 문장: 글마다 다르게 — 숫자 팩트 / 혼잣말 / 감탄사 / 질문 / 관찰 중 섞어서
- 길이: 짧은 글(150자 내외)과 긴 글(300자 내외)을 반반 섞어서
- 구조: 반전형뿐 아니라 나열형·고백형·단상형·팩트 나열형도 포함
- 줄바꿈: 매번 같은 위치에 줄바꿈하지 말 것
- 이모지: 없는 글, 마지막에만 있는 글, 중간에 있는 글 다양하게
